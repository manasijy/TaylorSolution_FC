...
for counter1=1:1:56
    m(counter1) = sqrt(6)*( (bh(counter1,1)*s_in(2,2))...
        - ((bh(counter1,2)*s_in(1,1))) + ((bh(counter1,4)*2*s_in(2,3)))...
        + ((bh(counter1,5)*2*s_in(3,1))) +((bh(counter1,6)*2*s_in(1,2))));
end   
Taylor_factor = max(m);
for counter2=1:1:56
    if m(counter2)==max(m)
...
for counter8=1:1:size(fivebyfive_mat,3)
    ...
    collect_all_valid_shear = [collect_all_valid_shear shear];
            if all(shear(:)>=0)
                if ismember((std(shear)),(sd_shear))
                    continue
                end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% Collect shear was taking shear of the first TBH solution it needs to be corrected%%%%%
%Shear has to correstpond to the active SS not any other
...
dummy2 = three_by_three(1,2,counter10)*collect_shear(counter10,5)*strain_increment;
e12 = dummy2+e12;
end
...
dummy2 = three_by_three(2,1,counter11)*collect_shear(counter11,5)*strain_increment;
e21 = dummy2+e21;
end
...
dummy2 = three_by_three(1,3,counter12)*collect_shear(counter12,5)*strain_increment;
e13 = dummy2+e13;
end
...
dummy2 = three_by_three(3,1,counter13)*collect_shear(counter13,5)*strain_increment;
e31 = dummy2+e31;
end
...
dummy2 = three_by_three(2,3,counter14)*collect_shear(counter14,5)*strain_increment;
e23 = dummy2+e23;
end
...
dummy2 = three_by_three(3,2,counter15)*collect_shear(counter15,5)*strain_increment;
e32 = dummy2+e32;
end
