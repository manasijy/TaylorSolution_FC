...
toc
       ...
       %% Modification
jj = 1;
slip(1).B = B; slip(1).shear = xb; slip(1).M = sum(xb);slip(1).N = N; slip(1).D = D; slip(1).r = r;
D1 = D;
yb1 = yb;
r1 = r;
...
for i = 1:1:l_s
        ...
        v1 = D1(p1,:)/d1(p1);              % row vector
      yb1= yb1 + v1'*( s(N(q1)) - d1'*s(B) );
      d1(p1)=d1(p1)-1;
      D1 = D1 - d1*v1;                  % update inverse basis matrix
%       t=B(p); B(p)=N(q);
...
%         yb= D'*s(B);
        r1 = s(N) - [A(:,N)]'*yb1;
        mxb= xb - xmin*d1; mxb(p1)=xmin;
        slip(jj).B = B; slip(jj).shear = mxb; slip(jj).M = sum(mxb);slip(jj).N = N; slip(jj).D = D1; slip(jj).r = r1;        
end
end
end
