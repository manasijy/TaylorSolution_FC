...
function [x,fval,it,B] = linprogWEB2(A,b,c,k,maxit,tol)
...
%%
B = n+1:n+m;                    % initial basis it contains no.s of vars in basis eg x6,x7,x8 etc
N = 1:n;                        % non-basis it contains no.s of vars in nonbasis eg x1,x2,x3,x4,x5 etc
...
if bmin<0,
   ...
   % one element is added with 1. xb is of the size of no of constraints. s is a
   % col with (total no of vars + no of slacks) zeros followed by (no of
   % equalities +1)ones. i.e. one extra equality is added to the set. New..
   % it seems they have added one extra var eg x2-x1 in place of x1 = -2
   % say.because x1 cant be -ve. the -ve var is pushed into NB.
...
elseif k==m,        % i.e. all inequalities
   ...
   phase=1; xb=abs(b); s=[zeros(n+k,1);ones(m-k,1)];   % supercost % For phase 1 the cost is sum of x_artificials which is to be minimized
end
while phase<3,
   df=-1; t=inf;
   ...
   while (it < maxit)
      ...
      r = s(N) - [A(:,N)]'*yb;      % reduced costs: r_q = c_q - c_B'*inv(B)*N_q ;Or r_q = c_q-pi'*Nq
                                    %Here r is a row vector so to get to
                                    %proper form we should transpose both
                                    %sides. r' = s(N)' - yb'*[A(:,N)
...
%          MS= find(abs(rmin)<= tol*(norm(s(N),inf)+1)); % multiple solution
%          if isempty(MS) break; % optimal! 
%          else
%       
%       
%       
%       
      ...
      if df>=0                      % apply Bland's rule to avoid cycling
         ...
         J=find(r<0); Nq=min(N(J)); q=find(N==Nq); %1-find all locations for which r is -ve 2)-Nq is the min of corresposnding  
                                                   %N values(Col nos) 3)- find the location of Nq in the array N- saved in q          
      end 
      ...
      I=find(d>tol); %    I=find(d>0); Here it says that if d <0 then solution is bounded. This is because sign on RHS is changed above
                     % I has the locations of positive d value
      if isempty(I), disp('Solution is unbounded'); it=-it; break; end
      xbd=xb(I)./d(I); [r,p]=min(xbd); p=I(p); % r is the step length alpha, p is position later p is the corresponding location of d value
      if df>=0,                     % apply Bland's rule to avoid cycling
         J=find(xbd==r); Bp=min(B(I(J))); p=find(B==Bp); 
      end 
      xb= xb - r*d; xb(p)=r;        % update x % here alpha*d is substracted %bsic var xb(p) is replaced with alpha 
